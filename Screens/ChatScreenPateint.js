...
const ChatScreenPateint = () => {
  ...
  useEffect(()=>{
    ...
    const functionn = async()=>{
        ...
        try {
         await database().ref('appointments').child(uid)
        ...
        console.log("doctos"+JSON.stringify(users));
        
            setAllusers(users)
            const snapshot = await database().ref('Patients/'+uid).get();
  ...
  const OpenGallery = ()=>{
    launchImageLibrary('photo' ,async (response)=>{
      ...
      setphoto(imagelink)
      const uid = auth().currentUser.uid
       await database().ref('Patients/'+uid).update({
        photo : imagelink
       })
...
export default ChatScreenPateint;
